...
namespace OneFileEncryptDecrypt.XCrypto
{
    ...
    {
        public static byte[] Encrypt(byte[] source, byte[] key, byte[] nonce, byte[] aad, string title, XModel.ProgressViewer? pv)
        {
        ...
        public static byte[] Decrypt(byte[] source, byte[] key, int nonceSize, byte[] aad, string title, XModel.ProgressViewer? pv)
        {
            using (var outStream = new MemoryStream())
            {
